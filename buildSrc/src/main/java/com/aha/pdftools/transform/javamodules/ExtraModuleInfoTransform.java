...
/**
 * An artifact transform that applies additional information to Jars without module information. The
 * transformation fails the build if a Jar does not contain information and no extra information was
 * defined for it. This way we make sure that all Jars are turned into modules.
 */
abstract public class ExtraModuleInfoTransform implements TransformAction<ExtraModuleInfoTransform.Parameter> {
    ...
    private static byte[] addModuleInfo(ModuleInfo moduleInfo) {
        ...
        ModuleVisitor moduleVisitor = classWriter.visitModule(moduleInfo.getModuleName(), Opcodes.ACC_OPEN, null);
